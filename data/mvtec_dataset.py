...
class MVTecDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            mask_origin = torch.zeros_like(image[:1])
            return {"img_aug": aug_image, "img_origin": image, "mask": aug_mask,"mask_normal":mask_origin}
        else:
            image = self.final_preprocessing(image)
            ...
            if base_dir == "good":
                ...
                mask_file_name = file_name.split(".")[0] + base_dir
            else:
                mask_path = os.path.join(dir_path, "../../ground_truth/")
                mask_path = os.path.join(mask_path, base_dir)
                mask_file_name = file_name_t.split(".")[0]  + "_mask.png"
                mask_path = os.path.join(mask_path, mask_file_name)
                mask = Image.open(mask_path)
                mask_file_name = file_name.split(".")[0]  + base_dir +"_mask.png"
                mask = self.mask_preprocessing(mask)
                mask = torch.where(
                    mask < 0.5, torch.zeros_like(mask), torch.ones_like(mask)
                )
            return {"img": image, "mask": mask,"img_name":file_name,"mask_name":mask_file_name}
